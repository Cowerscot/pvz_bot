...
def kb_stats(marketplace='yandex'):
    ...
    buttons = [f"📊 Статистика {mp_name}", f"🗑️ Очистить данные {mp_name}", f"🔄 Переподключить {mp_name}"]
    return _kb(*buttons)
...
def kb_main_menu():
    ...
    if MARKETPLACES.get('avito', {}).get('enabled'):
        buttons.append("🟢 Avito ПВЗ")
    buttons.append("🗑️ Очистить все данные")
    return _kb(*buttons)
...
def handle_marketplace_choice(user_id, peer_id, text, send_fn):
    ...
    # Ozon
        ...
        send_fn(peer_id, "🚧 Ozon в разработке...", kb_marketplace())
        return
    
    # Avito
    elif t == '🟢 авито пвз':
        send_fn(peer_id, "🚧 Avito в разработке...", kb_marketplace())
        return
    
    # Переподключение
    elif t == '🔄 переподключить яндекс':
        # Сначала очищаем cookies
